...
# https://static.wikia.nocookie.net/leagueoflegends/images/5/5a/Graduation_Day_LoR_profileicon.png/revision/latest/scale-to-width-down/80?cb=20210831213336
class QuotesSpider(scrapy.Spider):
    name = "summoner"

    def start_requests(self):
        urls = [
            'https://leagueoflegends.fandom.com/wiki/Summoner_icon_(Legends_of_Runeterra)',
        ]
        for url in urls:
            yield scrapy.Request(url=url, callback=self.parse)

    def parse(self, response):
        href_urls = response.xpath("//img").xpath("@src").getall()
        ...
        for href in href_urls:
            ...
            if "LoR_profileicon.png" in href:
                ...
                unquote_name = unquote_name.replace(".jpg", "")
